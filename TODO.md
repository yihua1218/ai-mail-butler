...
## High Priority
- [x] Implement built-in lightweight Rust SMTP server in `src/mail/mod.rs` (using `samotop`).
- [x] Set up database connection and migrations using `sqlx` in `src/db/mod.rs`.
- [x] Build Axum web server routing for API endpoints in `src/web/mod.rs`.
